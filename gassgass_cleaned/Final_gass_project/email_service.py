...
BRAND_NAME_MAP = {
    ...
    "HDO": "현대오일뱅크", "ETC": "알뜰/비브랜드", "RTO": "자영 알뜰주유소"
}
...
def get_nearby_stations():
    ...
    params = {
        ...
        "x": MY_X, "y": MY_Y,
        "radius": 5000, "sort": 2, "prodcd": "B027"
    }
...
def calculate_brand_stats(stations):
    ...
    for code, (total, count) in brand_stat.items():
        result.append({"name": BRAND_NAME_MAP.get(code, code), "code": code, "avg": total / count})
    return result


def send_scheduled_report() -> None:
    """스케줄러 자동발송 - 텍스트 메일 (전국평균 + 반경5km 최저가/최근접 + 브랜드별)"""
    ...
    for item in avg_list:
        diff_str = f"+{item['DIFF']}" if float(item['DIFF']) > 0 else str(item['DIFF'])
        national_text += f"[{item['PRODNM']}] 현재가: {item['PRICE']}원 (변동: {diff_str})\n"
    ...
    brand_text  = "".join([f"{b['name']}({b['code']}) 평균: {b['avg']:,.2f}원\n" for b in brand_stats])
...
def send_oil_summary(to_email: str, avg_list: list, low_top20_list: list, visit_count: int, news_list: list = []) -> None:
    ...
    if news_list:
        news_items = "".join([
            f"""<div style="padding:12px 0; border-bottom:1px solid #eee;">
                <a href="{news.get('link', '')}" style="font-weight:bold; color:#1e3a8a; text-decoration:none; font-size:15px;">{news.get('title', '')}</a>
                <p style="margin:5px 0 0; font-size:12px; color:#6b7280;">[{news.get('credit', '')}] {news.get('published', '')}</p>
                <p style="margin:5px 0 0; font-size:13px; color:#555;">{news.get('summary', '')[:100]}...</p>
            </div>"""
    ...
    if visit_count > 1:
        ...
        <div style="margin-top:40px; padding:20px; background-color:#1a1a1a; color:#00ff00;
            ...
            <p>사이트에 자주 방문할수록 화면에 보이는 기름값을 몰래 조금씩 인상하고 있었습니다.</p>
            <p>본 메일은 사용자 데이터가 기업의 이익을 위해 어떻게 조작될 수 있는지 보여주기 위한 <strong>팀 프로젝트</strong>입니다.</p>
        </div>"""
